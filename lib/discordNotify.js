...
export async function sendDiscordDm({ message, userId, url }) {
  // ── TEMP TEST OVERRIDE — route every DM to one test Discord user. ──────────
  // Replace <YOUR_DISCORD_ID> with your numeric Discord user ID.
  // To roll back: delete this block and change `targetUserId` back to `userId`.
  const TEST_DISCORD_ID = '1495322555190476940';
  const targetUserId = TEST_DISCORD_ID; // original: userId
  // ───────────────────────────────────────────────────────────────────────────
...
export function getIclosedSlaMinutes() {
  // ── TEMP TEST: 2-minute window for faster testing. Roll back to 5. ──
  const raw = parseInt(process.env.ICLOSED_SLA_MINUTES || '2', 10);
