/**
 * CRM-only Potential Leads pipeline stages (LT0–LT5) for UI display.
...
function hasLt0(row) {
  return Boolean(trimField(row?.phone)) && !trimField(row?.email);
}
...
export function computePotentialLeadLtStatus(row, options = {}) {
  ...
  if (hasLt0(row)) return LT_STATUS.LT0;
...
/**
 * Emails with a calls row where booking_origin ~ 'true' (LT5).
