...
const ACADEMIC_SUPABASE_URL = process.env.ACADEMIC_SUPABASE_URL;
const ACADEMIC_SUPABASE_KEY =
  process.env.ACADEMIC_SUPABASE_SERVICE_ROLE_KEY ||
  process.env.ACADEMIC_SUPABASE_SECRET_KEY ||
  process.env.ACADEMIC_SUPABASE_ANON_KEY;
...
function hasAcademicSupabaseConfig() {
  return Boolean(ACADEMIC_SUPABASE_URL && ACADEMIC_SUPABASE_KEY);
}

function getAcademicSupabaseClient() {
  return createClient(ACADEMIC_SUPABASE_URL, ACADEMIC_SUPABASE_KEY, {
    auth: {
...
function buildAcademicStatsPayload({
  avgAttendance,
  numberOfClasses,
  numberOfStudents,
  attendancePresent,
...
async function fetchAcademicStatsFromSupabase({ from, to, shiftBounds }) {
  const supabase = getAcademicSupabaseClient();

  const classRows = await fetchAllSupabaseRows(() =>
    supabase
      .from('classes')
      .select('id, date_time')
      .gte('date_time', from)
  ...
  if (classIds.length === 0) {
    return buildAcademicStatsPayload({
      avgAttendance: null,
      numberOfClasses: 0,
      numberOfStudents: 0,
      attendancePresent: 0,
  ...
  const attendanceRows = await fetchAllSupabaseRows(() =>
    supabase
      .from('attendance')
  ...
  const presentByClass = new Map();
  const distinctStudents = new Set();
  for (const row of attendanceRows) {
    if (!row.class_id || !row.student_id) continue;
    if (!presentByClass.has(row.class_id)) presentByClass.set(row.class_id, new Set());
    presentByClass.get(row.class_id).add(row.student_id);
    distinctStudents.add(row.student_id);
  }

  const attendancePresent = [...presentByClass.values()].reduce(
    (sum, presentStudents) => sum + presentStudents.size,
    0,
  );

  return buildAcademicStatsPayload({
    avgAttendance:
      classIds.length > 0 ? attendancePresent / classIds.length : attendancePresent > 0 ? attendancePresent : null,
    numberOfClasses: classIds.length,
    numberOfStudents: distinctStudents.size,
    attendancePresent,
    from,
    to,
  });
...
export default async function handler(req, res) {
  ...
  if (hasAcademicSupabaseConfig()) {
    try {
      const directStats = await fetchAcademicStatsFromSupabase({ from, to, shiftBounds });
      return res.status(200).json(directStats);
    } catch (err) {
      console.error('[academic-stats] Error fetching from Academic Supabase:', err.message);
      return res.status(200).json({
        ...
        attendanceTotal: null,
        showUpRate: null,
        error: err.message || 'Failed to fetch from Academic Supabase',
        window: { from, to, timezone: BUSINESS_TZ, shiftStartHourLocal: shiftStartHourLocal() },
        source: 'academic_supabase',
      });
    }
  }
