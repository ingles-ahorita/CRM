...
// ── Re-fetch a single transaction from Kajabi by ID ───────────────────────────
async function fetchTransactionById(txId) {
  try {
    const json = await kajabiFetch(`transactions/${txId}?filter[site_id]=${KAJABI_SITE_ID}`);
    return json.data ?? null;
  } catch (err) {
    console.warn(`[sync-kajabi] Could not re-fetch transaction ${txId}: ${err.message}`);
    return null;
  }
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ── 6. Re-check previously-failed transactions ──────────────────────────
    // A transaction can be synced as "failed" in month N, then succeed later.
    // Since created_at_kajabi doesn't change, syncing month N+1 won't re-fetch it.
    // Look back up to 60 days before the range start and re-fetch any failed rows.
    const SUCCESS_STATES = new Set(['paid', 'successful', 'success', 'complete', 'completed', 'succeeded']);
    const lookbackStart = new Date(new Date(fromISO).getTime() - 60 * 24 * 60 * 60 * 1000).toISOString();

    const { data: failedRows } = await supabase
      .from('kajabi_transactions')
      .select('kajabi_transaction_id, state')
      .gte('created_at_kajabi', lookbackStart)
      .lte('created_at_kajabi', toISO);

    const staleRows = (failedRows || []).filter(
      (r) => !r.state || !SUCCESS_STATES.has(r.state.toLowerCase())
    );

    let reResolvedCount = 0;
    if (staleRows.length > 0) {
      console.log(`[sync-kajabi] Re-checking ${staleRows.length} previously-failed transactions…`);
      const refreshedRows = [];

      for (const row of staleRows) {
        const fresh = await fetchTransactionById(row.kajabi_transaction_id);
        if (!fresh) continue;
        const attrs = fresh.attributes || {};
        const newState = attrs.state ?? null;
        if (newState && SUCCESS_STATES.has(newState.toLowerCase())) {
          refreshedRows.push({
            kajabi_transaction_id: String(fresh.id),
            kajabi_purchase_id:    txIdToPurchaseId[String(fresh.id)] ?? null,
            kajabi_offer_id:       fresh.relationships?.offer?.data?.id != null ? String(fresh.relationships.offer.data.id) : null,
            kajabi_customer_id:    fresh.relationships?.customer?.data?.id != null ? String(fresh.relationships.customer.data.id) : null,
            action:                attrs.action ?? null,
            state:                 newState,
            amount_in_cents:       attrs.amount_in_cents != null ? Number(attrs.amount_in_cents) : null,
            currency:              attrs.currency ?? 'USD',
            created_at_kajabi:     attrs.created_at ?? null,
            // Record when the flip was detected so revenue queries can attribute
            // this payment to the correct month (the month it actually resolved).
            payment_resolved_at:   now,
            raw:                   { attributes: fresh.attributes, relationships: fresh.relationships },
            synced_at:             now,
          });
        }
      }
      ...
      if (refreshedRows.length > 0) {
        const { error: refreshErr } = await supabase
          .from('kajabi_transactions')
          .upsert(refreshedRows, { onConflict: 'kajabi_transaction_id' });
        if (refreshErr) console.error('[sync-kajabi] Re-check upsert error:', refreshErr.message);
        else reResolvedCount = refreshedRows.length;
      }
      console.log(`[sync-kajabi] Re-resolved ${reResolvedCount} previously-failed transactions`);
    }
    ...
    return res.status(200).json({
      ...
      failed_now_resolved: reResolvedCount,
