...
export default async function handler(req, res) {
  ...
  try {
    ...
    // We bulk re-fetch the last 60 days from Kajabi (same endpoint, known to work)
    // and update any that flipped to a success state.
    ...
    const { data: failedRows } = await supabase
      ...
      .lt('created_at_kajabi', fromISO); // only prior-month transactions; current month already upserted above

    const staleIds = new Set(
      (failedRows || [])
        .filter((r) => !r.state || !SUCCESS_STATES.has(r.state.toLowerCase()))
        .map((r) => r.kajabi_transaction_id)
    );

    let reResolvedCount = 0;
    if (staleIds.size > 0) {
      console.log(`[sync-kajabi] Re-checking ${staleIds.size} stale transactions via bulk fetch (${lookbackStart} → ${fromISO})…`);

      const staleTxs = await fetchAllTransactionsInRange(lookbackStart, fromISO);
      const refreshedRows = [];

      for (const t of staleTxs) {
        if (!staleIds.has(String(t.id))) continue;
        const attrs = t.attributes || {};
      ...
      console.log(`[sync-kajabi] Re-resolved ${reResolvedCount}/${staleIds.size} previously-failed transactions`);
    }
