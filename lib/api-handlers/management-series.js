...
export default async function handler(req, res) {
  ...
  for (const dateStr of dateStrings) {
    ...
    try {
      // Same as generalStats: outcome_log with inner join to calls via call_id (same as closerStats/utmAnalytics), then dedupe and count with clawback rule
      const { data: outcomeLogs, error: outError } = await supabase
        .from('outcome_log')
      ...
      if (outError) {
        console.warn('[management-series] purchases error', dateStr, outError.message);
      } else if (outcomeLogs?.length) {
        ...
        // Count same as generalStats: 'yes' or refund with clawback < 100%
        totalPurchased = Array.from(byCallId.values()).filter((row) => {
          if (row.outcome === 'yes') return true;
          if (row.outcome === 'refund') {
            const clawbackPct = row.clawback ?? 100;
            return clawbackPct < 100;
          }
          return false;
        }).length;
      }
    } catch (e) {
      console.warn('[management-series] purchases exception', dateStr, e?.message);
    }
