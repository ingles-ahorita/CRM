...
// Format date/time for ManyChat messages (same format as initial bookings)
function formatDateTimeForManyChat(isoDateTime) {
...
export default async function handler(req, res) {
  ...
  const body = req.body || {};
  const { closerEmail, startDateTime, leadName, leadId, leadEmail, closerId, setterId, leadPhone, sourceType, mcApiKey, closer_mc_id, originalCallId } = body;
...
🏆 Aceptamos un número limitado de estudiantes en nuestra academia con Martin y solo seleccionamos los que realmente tienen ese compromiso para aprender inglés fluido.`;
    ...
    if (!supabase) {
      ...
      try {
        // New Google Meet link from the freshly created event (always use the latest link).
        const newCallLink =
          result?.data?.hangoutLink ||
          result?.data?.conferenceData?.entryPoints?.find((e) => e.entryPointType === 'video')?.uri ||
          null;

        // Load the original call row so the recovered duplicate preserves ALL of its data
        // (timezone, manychat ids, utm_*, calendly_questions, notes, etc.). Only the
        // recovery flags, scheduling fields, and closer-edited values change.
        ...
        if (originalCallId) {
          const { data: orig, error: origErr } = await supabase
            ...
            .single();
          if (origErr) {
            console.warn('[create-calendar-event] Could not load original call for preserve-copy:', origErr.message);
          } else {
            originalCall = orig;
          }
        }
        ...
        let callPayload;
        if (originalCall) {
          // Preserve everything from the original row; override only what a recovery changes.
          const preserved = { ...originalCall };
          delete preserved.id;          // let DB generate a new PK
          delete preserved.updated_at;  // let DB set its own timestamp
          callPayload = {
            ...preserved,
            // Closer-edited / scheduling changes
            closer_id: closerId,
            call_date: startDateTime,
            book_date: new Date().toISOString(),
            call_link: newCallLink,
            // Recovery flags — this is a fresh, active call
            recovered: true,
            is_reschedule: true,
            cancelled: false,
            // Google Calendar event: store the Google eventId here (same polymorphic
            // pattern as Calendly URI / iClosed numeric id). Drop the stale reschedule link.
            calendly_id: result?.data?.id || null,
            reschedule_link: null,
            // Reset outcomes: setter/closer will set these again on the new call
            picked_up: null,
            confirmed: null,
            showed_up: null,
            purchased: null,
          ...
          // Fallback (no original row available): previous minimal behavior, unchanged.
          callPayload = {
            lead_id: leadId,
            closer_id: closerId,
        ...
        if (callErr) {
          ...
          // Cancel the original call now that the recovered replacement exists (CRM-only).
          if (originalCallId) {
            const { error: cancelErr } = await supabase
              ...
              .eq('id', originalCallId);
            if (cancelErr) {
              console.warn('[create-calendar-event] Failed to cancel original call:', cancelErr.message);
            } else {
              console.log('[create-calendar-event] Original call cancelled:', originalCallId);
            }
          }
          ...
          if (subscriberId && mcApiKey) {
            ...
            try {
              const dateTime = formatDateTimeForManyChat(startDateTime);
                ...
                body: JSON.stringify({
                  ...
                  fieldsByName: [
                    ...
                    ...(dateTime ? [
                      ...
                      { name: 'call_time_local', value: dateTime.localTime },
                      { name: 'local_date', value: dateTime.localDate },
                    ] : []),
                  ],
                  apiKey: mcApiKey,
                }),
