...
function classifyIclosedCancelError(message) {
  const lower = String(message || '').toLowerCase();
  if (/already cancel/i.test(lower)) return 'ALREADY_CANCELED';
  if (/not found/i.test(lower)) return 'EVENT_CALL_NOT_FOUND';
  if (/numeric event call id/i.test(lower)) return 'MISSING_EVENT_ID';
  return 'UNKNOWN';
}
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return res.status(response.status).json({
        ...
        code: classifyIclosedCancelError(message),
        message,
      });
    }
