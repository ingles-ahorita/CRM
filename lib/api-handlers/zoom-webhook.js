...
// Supabase: use service role on the server so inserts into setter_calls succeed under RLS.
// Anon key is not authenticated as a user — INSERT policies on setter_calls typically deny it.
...
if (
  ...
  console.warn(
    '[zoom-webhook] SUPABASE_SERVICE_ROLE_KEY is not set; using anon key — setter_calls INSERT may be denied by RLS.'
  );
}
/**
