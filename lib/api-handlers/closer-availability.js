...
function sumIntervalsHours(intervals) {
  ...
  const mins = intervals
    ...
    .map((i) => {
      let fromMins = parseHm(i.from);
      let toMins = parseHm(i.to);
      if (toMins === 0 && fromMins > 0) toMins = MINUTES_PER_DAY;
  ...
  return utcWeekDates.map((dateStr) => {
    ...
    for (const sched of schedules) {
      ...
      for (const iv of intervals || []) {
        ...
        let fromMins = parseHm(iv.from);
        let toMins = parseHm(iv.to);
        if (toMins === 0 && fromMins > 0) toMins = 24 * 60;
