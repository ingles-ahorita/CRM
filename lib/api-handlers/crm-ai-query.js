...
async function toolGetRevenue({ month, from, to }) {
  ...
  const { data: txData, error: txErr } = await supabase
    ...
    .select('kajabi_transaction_id, kajabi_purchase_id, kajabi_offer_id, action, state, amount_in_cents, effective_date')
...
function buildSystemPrompt() {
  ...
  }).format(new Date()); // "YYYY-MM-DD" in business timezone
...
Today's date is ${today} (${BUSINESS_TIMEZONE} timezone). Always use this timezone when referring to dates and times in your answers — never show UTC times to the user.

## DATES AND TIMESTAMPS
- Revenue and payment dates come from Kajabi: use \`kajabi_transactions.created_at_kajabi\` (via get_revenue). These are the authoritative timestamps for when money was collected.
- Sales funnel dates (purchase_date in outcome_log) are CRM-entered and may differ slightly from Kajabi.
- When a user asks "when did X happen", prefer the Kajabi timestamp if it involves money, or the CRM date if it involves the sales process.
- Always present dates and times converted to ${BUSINESS_TIMEZONE} (e.g. "March 5 at 3:45 PM"), never as raw UTC ISO strings.
...
export default async function handler(req, res) {
  ...
  // Sanitize history: only user/assistant text turns, cap at 20 messages (~10 exchanges)
