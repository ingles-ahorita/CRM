 ...
 * Matches by email AND phone — both are required. Rows where is_reschedule is
 * true are excluded so a prior reschedule row is never picked as the original.
 * Returns the most recently scheduled matching row.
 */
async function findOriginalCallForReschedule(email, phone) {
  ...
  // neq silently drops NULLs in PostgreSQL; use .or() to include NULL and false.
  ...
  if (error) {
    console.warn('[iclosed-webhook] findOriginalCall error:', error.message);
    return { row: null };
  }

  const match = (rows || []).find((r) => String(r.phone || '').trim() === normPhone);
...
async function backfillCallsCalendlyId(email, phone, eventCallId) {
  ...
  if (normEmail) {
    const { data } = await supabase
      .from('calls')
      .select('id, calendly_id')
      .ilike('email', normEmail)
      .or('is_reschedule.is.null,is_reschedule.eq.false')
