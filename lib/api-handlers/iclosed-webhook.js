...
/**
 * Find the original (non-reschedule) calls row for an iClosed reschedule event.
 * Match strategy — three tiers, most precise first:
 *   Tier 1: call_date = oldStartTime AND email AND phone both match
 *   Tier 2: call_date = oldStartTime AND email matches
 *   Tier 3: call_date = oldStartTime AND phone matches
 * All tiers exclude existing reschedule rows (is_reschedule IS NOT TRUE).
 */
async function findOriginalCallForReschedule(oldStartTime, email, phone) {
  if (!oldStartTime) return { row: null, matchTier: null };

  const normEmail = email ? String(email).trim().toLowerCase() : null;
  const normPhone = phone ? String(phone).trim() : null;

  // neq('is_reschedule', true) silently drops NULL rows in PostgreSQL (<> does not match NULL).
  // Use .or() to include both false and NULL — original Zapier-created rows are typically NULL.
  const notRescheduled = 'is_reschedule.is.null,is_reschedule.eq.false';

  // Tier 1: call_date + email + phone
  if (normEmail && normPhone) {
    const { data: rows, error } = await supabase
      .from('calls')
      .select(CALLS_RESCHEDULE_SELECT)
      .eq('call_date', oldStartTime)
      .ilike('email', normEmail)
      .or(notRescheduled)
      .limit(5);

    if (error) {
      console.warn('[iclosed-webhook] findOriginalCall tier1 error:', error.message);
    } else {
      const match = (rows || []).find(
        (r) => String(r.phone || '').trim() === normPhone,
      );
      if (match) return { row: match, matchTier: 1 };
    }
  }

  // Tier 2: call_date + email only
  if (normEmail) {
    const { data: rows, error } = await supabase
      .from('calls')
      .select(CALLS_RESCHEDULE_SELECT)
    ...
    if (error) {
      console.warn('[iclosed-webhook] findOriginalCall tier2 error:', error.message);
    } else if (rows?.length) {
      return { row: rows[0], matchTier: 2 };
    }
  }

  // Tier 3: call_date + phone only
  if (normPhone) {
    const { data: rows, error } = await supabase
      .from('calls')
      .select(CALLS_RESCHEDULE_SELECT)
    ...
    if (error) {
      console.warn('[iclosed-webhook] findOriginalCall tier3 error:', error.message);
    } else if (rows?.length) {
      return { row: rows[0], matchTier: 3 };
    }
  }

  return { row: null, matchTier: null };
}
...
function buildRescheduledCallPayload(original, reschedule) {
  ...
  return {
    ...
    utm_term:                original.utm_term     ?? null,
    booking_origin:          original.booking_origin          ?? null,
    booking_origin_metadata: original.booking_origin_metadata ?? null,
...
const CALLS_RESCHEDULE_SELECT =
  ...
  'source_type, utm_source, utm_medium, utm_campaign, utm_term, ' +
  'booking_origin, booking_origin_metadata, name, email, phone, is_reschedule';
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isCallRescheduledWebhook(rawPayload)) {
      ...
      // Step 1: find original calls row by old_start_time + email/phone
      const { row: originalCall, matchTier } = await findOriginalCallForReschedule(
        reschedule.result.old_start_time,
      ...
      console.log('[iclosed-webhook] Call rescheduled: original call found', originalCall.id, 'tier', matchTier);

      // Step 2: backfill calendly_id on the original row if Zapier stored the wrong value
      let calendlyIdFixed = false;
      if (eventUuid && originalCall.calendly_id !== eventUuid) {
        const { error: fixErr } = await supabase
          .from('calls')
          .update({ calendly_id: eventUuid })
          .eq('id', originalCall.id);
        if (fixErr) {
          console.warn('[iclosed-webhook] calendly_id backfill failed:', fixErr.message);
        } else {
