...
export default async function handler(req, res) {
  ...
  try {
    ...
    // IMPORTANT (setter workflow):
    // Preserve existing metadata and only update the parts the setter can change via iClosed form.
    // We treat iClosed as the source of truth for answers, but we do NOT want other metadata keys to churn.
    const mergedMetadata = (() => {
      ...
      // Keep existing SLA annotations if present; do not recompute/merge other keys.
      const out = { ...base };
      ...
      // Optional: also preserve/refresh the embed/referrer URL only if it's missing.
      if (!out.Referrer_Url_Embed && incoming?.Referrer_Url_Embed) {
        out.Referrer_Url_Embed = incoming.Referrer_Url_Embed;
      }

      // Keep a lightweight normalized status trail if iClosed provides it, but do not overwrite other _normalized keys.
    ...
    const iclosedFields = {
      ...
      status: normalized,
      // Only overwrite these if iClosed sent a non-empty value; otherwise preserve existing.
    ...
    const assignment = await assignSetterForPotentialLead();
