  ...
  try {
    ...
    if (ICLOSED_BOOKED_STATUSES.has(normalized)) {
      ...
      if (isMetaSource(bookedSource)) {
        n8nFunnel = await sendFunnelEventToN8n({
          ...
          fbp: extracted.metadata?.tracking?._fbp || null,
        });
      }
    ...
    if (existing) {
      ...
      if (
        ...
        n8nFunnel = isMetaSource(updateLeadSource)
          ? await sendFunnelEventToN8n({
              event: 'lead',
              calendlyId: extracted.eventCallId,
          ...
          : { sent: false, skipped: true, reason: 'not_meta_source', source: updateLeadSource };
      }
    ...
    if (normalized === ICLOSED_STATUS.QUALIFIED) {
      n8nFunnel = isMetaSource(extracted.source)
        ? await sendFunnelEventToN8n({
            event: 'lead',
            calendlyId: extracted.eventCallId,
            email: extracted.email,
        ...
        : { sent: false, skipped: true, reason: 'not_meta_source', source: extracted.source || null };
    }
