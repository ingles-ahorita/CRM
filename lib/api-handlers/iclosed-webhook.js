...
export function extractContact(body) {
  ...
  const source =
    pickField(payload, [
      ...
      'contact.source',
      'tracking.utm_source',
      'tracking.first_utm_source',
    ]) ||
    // iClosed nests attribution under a top-level `tracking` object that is a
    // sibling of the resolved contact payload, so also probe the raw body.
    pickField(body, ['tracking.utm_source', 'tracking.first_utm_source']);
...
/**
 * Resolve the CRM closer that iClosed assigned on a reschedule, by email.
 * iClosed's `closerId` is its own internal id (not our uuid), so email is the
 * only reliable join. Closer emails are unique and email-less closers are
 * inactive, so a single case-insensitive email match is sufficient.
 * Returns { id, mc_api_key } or null when there's no email / no match.
 */
async function resolveCloserByEmail(email) {
  const clean = email ? String(email).trim() : '';
  if (!clean) return null;
  const { data, error } = await supabase
    .from('closers')
    .select('id, mc_api_key')
    .ilike('email', clean)
    .maybeSingle();
  if (error) {
    console.warn('[iclosed-webhook] resolveCloserByEmail error:', error.message);
    return null;
  }
  return data || null;
}
 ...
 *
 * `resolvedCloser` is the CRM closer iClosed reassigned the call to (looked up
 * by email). When it differs from the original closer, the new row adopts that
 * closer and its stale ManyChat subscriber id (`closer_mc_id`) is cleared so the
 * next confirm re-creates it against the NEW closer's bot. When null (no email /
 * no match), the original closer is preserved — never blanked.
 */
function buildRescheduledCallPayload(original, reschedule, resolvedCloser = null) {
  const eventUuid = reschedule.result.call_uuid ? String(reschedule.result.call_uuid) : null;
  const closerChanged = !!resolvedCloser?.id && resolvedCloser.id !== original.closer_id;
  return {
    // Stable fields copied from original
    lead_id:                 original.lead_id,
    closer_id:               resolvedCloser?.id ?? original.closer_id,
    setter_id:               original.setter_id,
    // Preserve the true first setter across reschedules (don't reset to current setter)
    first_setter_id:         original.first_setter_id ?? original.setter_id ?? null,
    source_type:             original.source_type,
    utm_source:              original.utm_source     ?? null,
    utm_medium:              original.utm_medium     ?? null,
    utm_campaign:            original.utm_campaign   ?? null,
    utm_content:             original.utm_content    ?? null,
    utm_campaign_CP:         original.utm_campaign_CP ?? null,
    booking_origin:          original.booking_origin ?? null,
    // Contact / CRM linkage that must survive a reschedule
    timezone:                original.timezone          ?? null,
    manychat_user_id:        original.manychat_user_id  ?? null,
    // Closer changed → the old closer's ManyChat subscriber id no longer applies;
    // clear it so confirm re-resolves against the new closer's bot.
    closer_mc_id:            closerChanged ? null : (original.closer_mc_id ?? null),
    calendly_questions:      original.calendly_questions ?? null,
...
const CALLS_RESCHEDULE_SELECT =
  'id, lead_id, closer_id, setter_id, first_setter_id, calendly_id, reschedule_link, call_date, book_date, ' +
  'source_type, utm_source, utm_medium, utm_campaign, utm_content, utm_campaign_CP, ' +
  'booking_origin, name, email, phone, timezone, manychat_user_id, closer_mc_id, ' +
  'calendly_questions, is_reschedule, cancelled, call_link';
 ...
 * @returns {{ row: object, matchKind: 'email'|'phone' } | null}
 */
async function findExistingByEmailOrPhone(email, phone) {
  ...
  if (normPhone) {
    ...
    if (error) {
      console.warn('[iclosed-webhook] phone dedupe lookup failed:', error.message);
    } else {
      const matched = (rows || []).filter((r) => phonesMatch(r.phone, normPhone));
      if (matched.length) {
...
// Only Meta-sourced leads should produce Meta CAPI funnel events. Per business
// convention paid Meta traffic is tagged utm_source = 'meta' (case-insensitive);
// facebook / instagram / etc. are intentionally NOT treated as Meta here.
function isMetaSource(source) {
  return String(source ?? '').trim().toLowerCase() === 'meta';
}

// Pull this lead's own click data straight from the inbound iClosed payload's
// `tracking` block + top-level `ipAddress`. Using the same request guarantees
// the fbclid/fbc/ip belong to THIS lead (no cross-wiring from a separate lookup).
// `_fbc` is already a valid `fb.1.<clickTime>.<fbclid>`, so Meta gets a correct
// creationTime when it's forwarded verbatim.
function clickDataFromExtracted(extracted) {
  ...
  return {
    ...
    fbp: t._fbp || null,
    ipAddress: meta.ipAddress || meta._normalized?.ip_address || null,
  };
}
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (isCallRescheduledWebhook(rawPayload)) {
      ...
      // iClosed can reassign the closer on a reschedule — resolve the new closer
      // by email so the CRM row reflects it (was previously copying the old closer).
      const rescheduleCloserEmail = reschedule.summary?.call?.closerEmail || null;
      const resolvedCloser = await resolveCloserByEmail(rescheduleCloserEmail);
      const closerChanged = !!resolvedCloser?.id && resolvedCloser.id !== originalCall.closer_id;
      if (rescheduleCloserEmail && !resolvedCloser) {
        console.warn(
          '[iclosed-webhook] Call rescheduled: no CRM closer matched email',
          rescheduleCloserEmail, '— keeping original closer', originalCall.closer_id,
        );
      } else if (closerChanged) {
        console.log(
          '[iclosed-webhook] Call rescheduled: closer reassigned',
          originalCall.closer_id, '→', resolvedCloser.id, `(${rescheduleCloserEmail})`,
        );
      }

      const newCallPayload = buildRescheduledCallPayload(originalCall, reschedule, resolvedCloser);
      ...
      await logWebhook({
        ...
        result: {
          ...
          original_call_id: originalCall.id,
          calendly_id_fixed: calendlyIdFixed,
          closer_email: rescheduleCloserEmail,
          closer_reassigned: closerChanged,
          new_closer_id: resolvedCloser?.id ?? originalCall.closer_id,
          closer_match: rescheduleCloserEmail ? (resolvedCloser ? 'matched' : 'no_match') : 'no_email',
        },
      });
      return respond(res, 200, {
        received: true,
    ...
    if (isCallCancelledWebhook(rawPayload)) {
      ...
      // Mark cancelled and force confirmed → NO (mirrors the CRM-side cancel in
      // LeadItem.jsx and the ManyChat confirmed=false update below). Writing
      // confirmed=false when it's already false is a no-op for the platform_events
      // trigger (IS DISTINCT FROM), so no guard is needed.
      const { data: cancelledRows, error: cancelErr } = await supabase
        .from('calls')
        .update({ cancelled: true, confirmed: false })
        .eq('calendly_id', eventUuid)
        .select('id, manychat_user_id, confirmed');
    ...
    if (ICLOSED_BOOKED_STATUSES.has(normalized)) {
      // Meta funnel "schedule" is sent further below — only for Meta-sourced
      // leads (utm_source = 'meta'). The booked payload itself doesn't carry
      // utm_source, so we read the source persisted on the lead at ingest, which
      // means we must resolve the potential-lead row first.
      ...
      if (!existingBooked) {
        // No lead row → can't validate the source → don't send the funnel event.
        await logWebhook({
          event: extracted.event,
          status_in: extracted.statusRaw,
          process: 'skipped',
          result: {
            ...
            normalized,
            n8n_funnel: { sent: false, reason: 'no_potential_lead_row' },
      ...
      // Meta funnel: booked call → "schedule", but ONLY for Meta-sourced leads.
      // The booked payload lacks utm_source, so use the source persisted on the
      // lead at ingest; when it's not 'meta' (incl. unknown/legacy null), skip
      // the send (fail-closed).
      const bookedSource = existingBooked.source || extracted.source || null;
      let n8nFunnel = { sent: false, skipped: true, reason: 'not_meta_source', source: bookedSource };
      if (isMetaSource(bookedSource)) {
        const click = clickDataFromExtracted(extracted);
    ...
    if (existing) {
      ...
      // Meta funnel: first transition to qualified → "lead", but ONLY for
      // Meta-sourced leads (utm_source = 'meta'). Re-fired qualified webhooks
      // (row already qualified) send nothing. Best-effort; response unchanged.
      const updateLeadSource = extracted.source || existing.source || null;
      let n8nFunnel = null;
      if (
        normalized === ICLOSED_STATUS.QUALIFIED &&
        rowIclosedStatus(existing) !== ICLOSED_STATUS.QUALIFIED
      ) {
        if (isMetaSource(updateLeadSource)) {
          const click = clickDataFromExtracted(extracted);
          n8nFunnel = await sendFunnelEventToN8n({
            event: 'lead',
            calendlyId: extracted.eventCallId,
          ...
          n8nFunnel = { sent: false, skipped: true, reason: 'not_meta_source', source: updateLeadSource };
        }
      }
    ...
    // Meta funnel: contact created directly as qualified → "lead", but ONLY for
    // Meta-sourced leads (utm_source = 'meta').
    let n8nFunnel = null;
    if (normalized === ICLOSED_STATUS.QUALIFIED) {
      if (isMetaSource(extracted.source)) {
        const click = clickDataFromExtracted(extracted);
        n8nFunnel = await sendFunnelEventToN8n({
          event: 'lead',
          calendlyId: extracted.eventCallId,
        ...
        n8nFunnel = { sent: false, skipped: true, reason: 'not_meta_source', source: extracted.source || null };
      }
    }
