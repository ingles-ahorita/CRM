...
export async function processIclosedPotentialLeadSlaNotifications() {
  ...
  // one-time backfill, so only future leads notify "from now on". Each lead is
  // DMed its assigned setter (setters.discord_id).
  ...
  for (const row of rows || []) {
    ...
    // Only notify leads that compute to LT0/LT2/LT3 (has phone, not yet booked).
    const lt = computePotentialLeadLtStatus(row);
    if (lt !== LT_STATUS.LT0 && lt !== LT_STATUS.LT2 && lt !== LT_STATUS.LT3) {
      results.skipped += 1;
      continue;
    }
