...
export default async function handler(req, res) {
  ...
  try {
    ...
    const isInvalidDimensionError = (error) => /not a valid dimension|invalid dimension|please remove .*dimension|field .* is not valid/i.test(error?.message || '');
    ...
    for (const attemptDimensions of dimensionAttempts) {
      try {
        ...
        if (!customDimensions.length || !isInvalidDimensionError(reportErr)) throw reportErr;
