...
export default async function handler(req, res) {
  ...
  const requestedPagePath = (req.query?.pagePath || '').trim();
  const pagePath = requestedPagePath || '/';
  const hasExplicitPagePath = !!requestedPagePath;
  ...
  const filterDimension = (req.query?.filterDimension || '').trim();
  const filterValue = (req.query?.filterValue || '').trim();
  const dimensionsParam = (req.query?.dimensions || '').trim();
  const metricNameParam = (req.query?.metricName || '').trim();
  const customDimensions = dimensionsParam
  ...
  console.log('[google-analytics] Request:', {
    ...
    metricName: metricNameParam || '(default)',
    dimensions: customDimensions.length ? customDimensions : '(default)',
    wholeSite,
    startDate,
    endDate,
  ...
  try {
    ...
    } else if (pagePath && (!customDimensions.length || hasExplicitPagePath)) {
      pagePathFilter = {
        filter: {
          fieldName: 'pagePath',
    ...
    // If eventName is provided, we combine it with the page filter
    let combinedFilter = pagePathFilter;
    if (eventName) {
      ...
      if (filterDimension && filterValue) {
        eventExpressions.push({
          filter: {
            fieldName: filterDimension,
            stringFilter: { matchType: 'EXACT', value: filterValue, caseSensitive: false }
          }
        });
      }

      const eventFilter = eventExpressions.length > 1
        ? { andGroup: { expressions: eventExpressions } }
        : eventExpressions[0];

      combinedFilter = pagePathFilter
        ? { andGroup: { expressions: [pagePathFilter, eventFilter] } }
        : eventFilter;
    }

    const dimensionVariants = (name) => {
      if (name === 'video_percent') return ['video_percent', 'customEvent:video_percent'];
      return [name];
    };
    const buildDimensionAttempts = (dimensions) => {
      if (!dimensions.length) return [[]];
      return dimensions.reduce((attempts, dimensionName) => {
        const variants = dimensionVariants(dimensionName);
        return attempts.flatMap((attempt) => variants.map((variant) => [...attempt, variant]));
      }, [[]]);
    };
    /** Re-throw only for auth; other GA errors on custom-dimension reports degrade to 200 + unavailable. */
    const isCredentialPermissionError = (error) => {
      const msg = String(error?.message || '');
      const code = error?.code ?? error?.status ?? error?.statusCode;
      const codeStr = code != null ? String(code) : '';
      return /credential|authentication|UNAUTHENTICATED|PERMISSION_DENIED|not granted|access was denied/i.test(msg)
        || /UNAUTHENTICATED|PERMISSION_DENIED/.test(codeStr)
        || code === 7 || code === 16
        || code === 403 || code === 401;
    };

    // Run main report (views/sessions or caller-specified metric + dimensions)
    const reportMetric = metricNameParam || (useSessions ? 'sessions' : 'screenPageViews');
    const defaultDimensions = ['date', 'pagePath'];
    const requestedDimensions = customDimensions.length > 0 ? customDimensions : defaultDimensions;
    const dimensionAttempts = buildDimensionAttempts(requestedDimensions);
    let viewsReportResult;
    let usedDimensions = requestedDimensions;
    let lastDimensionError = null;

    for (const attemptDimensions of dimensionAttempts) {
      try {
        const [result] = await client.runReport({
          property,
          dateRanges: [{ startDate, endDate }],
          dimensions: attemptDimensions.map((name) => ({ name })),
          dimensionFilter: combinedFilter,
          metrics: [{ name: reportMetric }],
        });
        viewsReportResult = result;
        usedDimensions = attemptDimensions;
        break;
      } catch (reportErr) {
        lastDimensionError = reportErr;
        if (!customDimensions.length) throw reportErr;
        if (isCredentialPermissionError(reportErr)) throw reportErr;
        // Try next dimension API name (e.g. video_percent vs customEvent:video_percent), then 200 unavailable.
      }
    }

    if (!viewsReportResult && customDimensions.length > 0) {
      return res.status(200).json({
        rows: [],
        startDate,
        endDate,
        ...
        unavailable: true,
        reason: lastDimensionError?.message || 'Requested GA dimension is unavailable for this property.',
        hint: customDimensions.includes('video_percent')
          ? 'GA4 video_percent must be registered as an event-scoped custom dimension, usually available to the Data API as customEvent:video_percent. It is not historical; data starts after registration.'
          : 'Check the GA4 Data API name for this dimension in the property metadata.',
      });
    }

    if (customDimensions.length > 0) {
      const rows = (viewsReportResult.rows || []).map((row) => {
        const dimensions = {};
        customDimensions.forEach((name, idx) => {
          dimensions[name] = row.dimensionValues?.[idx]?.value ?? '';
        });
        return {
          dimensions,
          metric: Number(row.metricValues?.[0]?.value || 0) || 0,
        };
      });
