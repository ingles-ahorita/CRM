...
export default async function handler(req, res) {
  ...
  try {
    ...
    const { data: offer, error: offerError } = await supabase
      .from('offers')
      .select('kajabi_id, weekly_classes, is_subscription')
    ...
    console.log(
      `Found offer: kajabi_id=${offerId}, weekly_classes=${weeklyClasses}, is_subscription=${isSubscription}`
    );
    ...
    if (admin) {
      const cents = payload.payload?.amount_paid ?? payload.payload?.subtotal;
      const amt =
        cents != null ? `$${(Number(cents) / 100).toFixed(2)}` : '';
      const txId =
        payload.payload?.transaction_id ||
        payload.payload?.subscription_id ||
        storedPayloadId;
      try {
        await logPlatformEvent(admin, {
          event_type: 'kajabi.purchase',
          category: 'sale',
          priority: 3,
          summary: `Kajabi purchase ${amt} — ${customerName} (${memberEmail})`,
          actor_type: 'webhook',
          actor_display: 'Kajabi',
          lead_email: memberEmail,
          lead_name: customerName,
          source: 'kajabi',
          dedupe_key: txId ? `kajabi:purchase:${txId}` : null,
          metadata: { offer_id: offerId },
        });
      } catch (logErr) {
        console.warn('[kajabi-webhook] platform_events:', logErr?.message);
      }
    }
