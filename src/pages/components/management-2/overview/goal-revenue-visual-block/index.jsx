...
function SemiCircleGauge({
  ...
  return (
    <div className="relative shrink-0" style={{ width, height: 90 }}>
      <svg
        width={width}
        height={svgH}
        viewBox={`0 0 ${width} 108`}
        className="block overflow-visible -mt-[50px]"
        aria-hidden
      >
        <path
          d={arcPath}
      ...
      <div className="pointer-events-none absolute left-1/2 top-[62%] flex w-[min(140px,90%)] -translate-x-1/2 -translate-y-[55%] flex-col items-center text-center">
        <div className="text-[30px] font-extrabold leading-none tracking-tight text-slate-900 tabular-nums">
          {labelMain}
        </div>
...
export default function GoalRevenueVisualBlock() {
  ...
  useEffect(() => {
    ...
    async function loadBlockData() {
      ...
      try {
        const now = new Date();
        const mtdRange = DateHelpers.getMonthRangeInTimezone(
          now,
        ...
        const [mtdTxRes, weeklyTxRes] = await Promise.all([
          supabase
            ...
            .select(
              "amount_in_cents, action, state, effective_date, payment_resolved_at",
            )
            .or(
        ...
        const mtdNet = (mtdTxRes.data || []).reduce((sum, t) => {
          const resolvedInThisMonth =
            t.payment_resolved_at != null &&
            t.payment_resolved_at >= mtdStartISO &&
            t.payment_resolved_at <= mtdEndISO &&
            (t.effective_date == null ||
              t.effective_date < mtdStartISO ||
              t.effective_date > mtdEndISO);
          const action = resolvedInThisMonth
            ? "charge"
            : (t.action ??
              ((t.amount_in_cents || 0) >= 0 ? "charge" : "refund"));
          const isRefund =
            action === "refund" || Number(t.amount_in_cents || 0) < 0;
          const isDispute = action === "dispute";
          const isFailed =
            !resolvedInThisMonth &&
            (isDispute ||
              (t.state != null &&
                !SUCCESS_STATES.includes(String(t.state).toLowerCase())));
        ...
        const weekBounds = [3, 2, 1, 0].map((offset) =>
          DateHelpers.getWeekBoundsForOffset(offset),
        );
        const grouped = weekBounds.map((w, idx) => {
          const rows = (weeklyTxRes.data || []).filter((r) => {
            const ts = r?.created_at_kajabi
              ? new Date(r.created_at_kajabi).getTime()
              : 0;
            return ts >= w.weekStart.getTime() && ts <= w.weekEnd.getTime();
          });
          return {
            w: idx,
            v: sumNetFromTransactions(rows),
            weekLabel: formatWeekRangeLabel(w.weekStart, w.weekEnd),
          };
        });
  ...
  return (
    <div className="border border-slate-200 rounded-2xl p-2 bg-white">
      ...
      <div className="grid grid-cols-1 gap-3">
        <div className="flex flex-col border border-slate-200 bg-white p-2 shadow-[0_1px_2px_rgba(15,23,42,0.05)] rounded-xl">
          <div className="flex items-start justify-between gap-2 pb-4">
            <div className="text-[14px] font-medium tracking-wide text-black">
          ...
          <div className="flex flex-1 flex-col gap-2 sm:flex-row sm:items-stretch">
            ...
            <div className="flex min-w-0 flex-1 flex-col justify-center gap-1 pl-0 sm:pl-2">
              {loading ? (
                <>
                  {shimmer("h-8 w-48")}
                  {shimmer("h-4 w-44")}
                ...
                <>
                  ...
                  <div className="text-[10px] font-medium text-slate-500">
                    Day {dayOfMonth}/{daysInMonth} • target{" "}
                    {formatUsd(expectedTarget)}
                  </div>
                  <div>
                    <span
                      className={cx(
                        "inline-flex w-[150px] rounded-xl px-3 py-0.5 text-[10px] font-bold ring-1",
                        paceClass,
                      )}
                    >
                      {paceText}
        ...
        <div className="flex flex-col rounded-xl border border-slate-200 bg-white p-4 shadow-[0_1px_2px_rgba(15,23,42,0.05)]">
          <div className="flex items-start justify-between gap-2 pb-3">
            <div className="text-[14px] font-medium uppercase tracking-wide text-black">
              NET REVENUE TREND
            </div>
              ...
              <ResponsiveContainer width="100%" height="100%" minHeight={100}>
                ...
                >
                  ...
                  <Tooltip
                    ...
                    formatter={(value) => [formatUsd(value), "Net revenue"]}
          ...
          {loading ? (
            ...
            <div className="mt-2 text-[12px] font-medium text-slate-500">
              {trendDelta >= 0 ? "+" : "-"}
              {formatUsd(Math.abs(trendDelta))} vs prior week
            </div>
          )}
        </div>
      </div>
