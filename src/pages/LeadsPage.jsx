...
export default function LeadsPage() {
  ...
  useEffect(() => {
    ...
    if (headerState.activeTab !== "today")
      params.set("tab", headerState.activeTab);
    if (headerState.sortBy !== "book_date")
      params.set("sortBy", headerState.sortBy);
    if (headerState.sortOrder !== "desc")
      params.set("sortOrder", headerState.sortOrder);
    if (headerState.startDate) params.set("start", headerState.startDate);
    if (headerState.endDate) params.set("end", headerState.endDate);
    if (headerState.setterFilter)
      params.set("setter", headerState.setterFilter);
    if (headerState.closerFilter)
      params.set("closer", headerState.closerFilter);
    if (headerState.transferred)
      params.set("transferred", headerState.transferred);
              ...
              {headerState.sortBy === "call_date" && (
                <>
                  ...
                  <span style={{ fontSize: "14px", color: "#374151" }}>
                    ...
                    {(
                      (dataState.counts.confirmed /
                        slots[dataState.currentDate]) *
                      100
                    ).toFixed(2)}
                    %
                  </span>
